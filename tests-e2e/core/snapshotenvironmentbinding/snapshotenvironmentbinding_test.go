package snapshotenvironmentbinding
...
var _ = Describe("SnapshotEnvironmentBinding Reconciler E2E tests", func() {
	...
	Context("Testing SnapshotEnvironmentBinding Reconciler.", func() {
		...
		It("Should update Status of Binding and create new GitOpsDeployment CR.", func() {
			...
			binding := bindingFixture.BuildSnapshotEnvironmentBindingResource("appa-staging-binding", "new-demo-app", "staging", "my-snapshot", 3, []string{"component-a", "component-b"})

			err = k8s.Create(&binding, k8sClient)
			Expect(err).To(Succeed())

			// Update Status field
			err = bindingFixture.BuildAndUpdateBindingStatus(binding.Spec.Components,
		...
		It("Should update GitOpsDeployment CR if Binding CR is updated.", func() {
			...
			binding := bindingFixture.BuildSnapshotEnvironmentBindingResource("appa-staging-binding", "new-demo-app", "staging", "my-snapshot", 3, []string{"component-a"})
			err = k8s.Create(&binding, k8sClient)
			Expect(err).To(Succeed())
