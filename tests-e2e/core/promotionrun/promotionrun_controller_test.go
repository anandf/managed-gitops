...
var _ = Describe("Application Promotion Run E2E Tests.", func() {
	Context("Testing Application Promotion Run Reconciler.", func() {
		...
		BeforeEach(func() {
			...
			err = bindingFixture.BuildAndUpdateBindingStatus(bindingStage.Spec.Components,
			...
			bindingProd = bindingFixture.BuildSnapshotEnvironmentBindingResource("appa-prod-binding", "new-demo-app", "prod",
				"my-snapshot", 3, []string{"component-a"})
			err = k8s.Create(&bindingProd, k8sClient)
			Expect(err).To(Succeed())

			By("Update Status field.")
			err = bindingFixture.BuildAndUpdateBindingStatus(bindingProd.Spec.Components,
				"https://github.com/redhat-appstudio/managed-gitops", "main", "fdhyqtw",
